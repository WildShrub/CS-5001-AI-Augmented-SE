...
from .prompts import program_prompt, tests_prompt, improve_tests_prompt
from .tools import Tools
from .types import AgentConfig, RunResult
from .utils import parse_coverage_total
...
class Agent:
    ...
    def create_program(self, desc: str, module_path: str) -> RunResult:
        ...
        content = self.llm.generate(program_prompt(desc, existing))
        if not content.strip():
            return RunResult(False, "Model returned empty module.")
        self.tools.write(module_path, content.strip() + "\n")
    ...
    def create_tests(self, desc: str, module_path: str, tests_path: str) -> RunResult:
        ...
        content = self.llm.generate(tests_prompt(desc, module_path, module_code, existing_tests))
        if not content.strip():
            return RunResult(False, "Model returned empty tests.")
        self.tools.write(tests_path, content.strip() + "\n")
    ...
    def improve_tests_to_100(self, desc: str, module_path: str, tests_path: str, max_iters: int) -> RunResult:
        for i in range(1, max_iters + 1):
            ok, out, total, cov_data = self._run_tests_with_coverage()
            missing = _missing_lines_summary(cov_data, module_path)
            if ok and total >= 100.0:
                return RunResult(True, f"Tests passed and coverage is 100%. Iterations used: {i}.", coverage=total)
            ...
            if not new_tests.strip():
                return RunResult(False, f"Iteration {i}: model returned empty tests. Coverage={total:.1f}.\nLast output:\n{out}", coverage=total)
            self.tools.write(tests_path, new_tests.strip() + "\n")

        ok, out, total, _ = self._run_tests_with_coverage()
        status = "passed" if ok else "failed"
        return RunResult(False, f"Stopped at max iterations. Final tests {status}. Coverage={total:.1f}.\nLast output:\n{out}", coverage=total)
