...
from .prompts import coverage_target_prompt, improve_tests_prompt, program_prompt, tests_prompt
...
class Agent:
    ...
    def parse_coverage_target(self, text: str) -> float:
        raw = self.llm.generate(coverage_target_prompt(text)).strip()

        m = re.search(r"(\d+(?:\.\d+)?)", raw)
        if not m:
            raise ValueError(f"LLM did not return a number for coverage target. Got: {raw!r}")

        val = float(m.group(1))

        # If model returned a fraction (0.95), treat as 95%.
        if 0.0 <= val <= 1.0:
            val *= 100.0

        if not (0.0 <= val <= 100.0):
            raise ValueError(f"Coverage target out of range after parsing: {val} (raw={raw!r})")
